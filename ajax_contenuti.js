...
//Per il caricamento dei dati dinamicamente faccio una fetch al file content.php
fetch('http://localhost/prova/content.php').then(onResponse, onError).then(onJson);
...
function Preferiti(){
	...
	const urlprefe='http://localhost/prova/preferiti.php?id=' + identificativo + '&titolo=' + titolo + "";
	//console.log(urlprefe);
	fetch(urlprefe).then(onResponse2, onError2);
...
function aggiornapreferiti(){//la funzione aggiornapreferiti dovrebbe funzio
	fetch('http://localhost/prova/agg_prefe.php').then(onResponse, onError).then(onJson3);
}
